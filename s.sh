...
# set -euxo pipefail
# File: adbhostgen.sh
VERSION="2018.07.10.01"
HomePage="http://gg.gg/ADB_"
# where ads go to die
...
printHelp ()
{
	...
	printf '\t'; echo -n "[-1]"; printf '\t\t\t\t'; echo "BLITZ=1: increased protection"
	printf '\t'; echo -n "[-2]"; printf '\t\t\t\t'; echo "BLITZ=2: optimum protection"
	printf '\t'; echo -n "[-3]"; printf '\t\t\t\t'; echo "BLITZ=3: unlock maximum protection [DEFAULT]"
...
selfUpdate ()
{
	...
	if ping -q -c 1 -W 1 google.com >/dev/null; then
		MPGETSSL ${HomePage}/$(basename "$0") > $TMPFILE
...
if [ $ONLINE -eq 1 ] && ping -q -c 1 -W 1 google.com >/dev/null; then
	...
	MPGETSSL https://raw.githubusercontent.com/oznu/dns-zone-blacklist/master/dnsmasq/dnsmasq.blacklist | grep -o '^[^#]*' | grep -v "::" | sed 's/0.0.0.0$/'$SetIP'/' > $tmpdomains
	MPGETSSL https://raw.githubusercontent.com/notracking/hosts-blocklists/master/domains.txt | grep -o '^[^#]*' | grep -v "::" | sed 's/0.0.0.0$/'$SetIP'/' >> $tmpdomains
	MPGETSSL -d mimetype=plaintext -d hostformat=dnsmasq https://pgl.yoyo.org/adservers/serverlist.php? | grep -o '^[^#]*' | grep -v "::" | sed 's/127.0.0.1$/'$SetIP'/' >> $tmpdomains
	...
	if [ $BLITZ -ge 1 ]; then
		...
		MPGETSSL https://raw.githubusercontent.com/FadeMind/hosts/master/extensions/porn/sinfonietta/hosts | grep -o '^[^#]*' | awk '{print $2}' >> $tmphosts
	...
	if [ $BLITZ -ge 2 ]; then
		...
		MPGETSSL http://sbc.io/hosts/alternates/fakenews-gambling-porn/hosts | grep -o '^[^#]*' | awk '{print $2}' >> $tmphosts
		...
		MPGETSSL https://raw.githubusercontent.com/crazy-max/WindowsSpyBlocker/master/data/hosts/spy.txt | grep -o '^[^#]*' | awk '{print $2}' >> $tmphosts
	...
	if [ $NOFB = "f" ]; then
		...
		MPGETSSL ${HomePage}/blacklists/facebookonly.block >> $tmphosts
	fi

	if [ $NOFB = "F" ]; then
		lognecho "> Blocking Facebook, Messenger, Instagram, WhatsApp"
		MPGETSSL ${HomePage}/blacklists/facebookall.block >> $tmphosts
	fi

	lognecho "> Updating official blacklist/whitelist files"
	MPGETSSL ${HomePage}/blacklists/blacklist | grep -o '^[^#]*' > $blacklist
	MPGETSSL ${HomePage}/whitelists/whitelist | grep -o '^[^#]*' > $whitelist
	MPGETSSL ${HomePage}/whitelists/fruitydomains > $base64wl
