...
#___________________________________________________________________________________________________________________________________________________________________________________________________
	...
	MPGETSSL ${u04} | grep -o '^[^#]*' | awk '{print $2}' > $hTam
	lognecho "> Dang tai: notracking blocklists"
	MPGETSSL ${u05} | grep -o '^[^#]*' | grep -v "::" | awk '{print $2}' >> $hTam
	lognecho "> Dang tai: Disconnect.me lists"
	MPGETSSL ${u06} | grep -o '^[^#]*' >> $hTam
	MPGETSSL ${u07} | grep -o '^[^#]*' >> $hTam
	MPGETSSL ${u08} | grep -o '^[^#]*' >> $hTam
	MPGETSSL ${u09} | grep -o '^[^#]*' >> $hTam
	lognecho "> Dang tai: quidsup/notrack lists"
	MPGETSSL ${u10} | grep -o '^[^#]*' >> $hTam
	MPGETSSL ${u11} | grep -o '^[^#]*' >> $hTam
	lognecho "> Dang tai: MalwareDomains lists"
	MPGETSSL ${u12} | grep -o '^[^#]*' >> $hTam
	MPGETSSL ${u13} | grep -o '^[^#]*' >> $hTam
	lognecho "> Dang tai: abuse.ch blocklists"
	MPGETSSL ${u14} | grep -o '^[^#]*' >> $hTam
	lognecho "> Dang tai: Ransomware blocklists"
	MPGETSSL ${u15} | grep -o '^[^#]*' >> $hTam
	MPGETSSL ${u16} | grep -o '^[^#]*' >> $hTam
	MPGETSSL ${u17} | grep -o '^[^#]*' >> $hTam
	MPGETSSL ${u18} | grep -o '^[^#]*' >> $hTam
	lognecho "> Dang tai: adaway list"
	MPGETSSL ${u19} | grep -o '^[^#]*' | grep -v "::" | awk '{print $2}' >> $hTam
#___________________________________________________________________________________________________________________________________________________________________________________________________
	if [ $Level -ge 1 ]; then
		lognecho "# Unlocking Level=1 lists"
		lognecho "> Dang tai: more StevenBlack lists"
		MPGETSSL ${u20} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		MPGETSSL ${u21} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		MPGETSSL ${u22} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		MPGETSSL ${u23} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		lognecho "> Dang tai: hosts-file ATS/EXP/GRM lists"
		MPGETSSL ${u24} | grep -o '^[^#]*' | grep -v "::" | awk '{print $2}' >> $hTam
		MPGETSSL ${u25} | grep -o '^[^#]*' | grep -v "::" | awk '{print $2}' >> $hTam
		MPGETSSL ${u26} | grep -o '^[^#]*' | grep -v "::" | awk '{print $2}' >> $hTam
		lognecho "> Dang tai: hosts-file HJK/PUP lists"
		MPGETSSL ${u27} | grep -o '^[^#]*' | grep -v "::" | awk '{print $2}' >> $hTam
		MPGETSSL ${u28} | grep -o '^[^#]*' | grep -v "::" | awk '{print $2}' >> $hTam
		lognecho "> Dang tai: dshield lists"
		MPGETSSL ${u29} | grep -o '^[^#]*' >> $hTam
		MPGETSSL ${u30} | grep -o '^[^#]*' >> $hTam
		MPGETSSL ${u31} | grep -o '^[^#]*' >> $hTam
		lognecho "> Dang tai: pgl.yoyo.org list"
		MPGETSSL -d mimetype=plaintext -d hostformat=unixhosts ${u32} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		lognecho "> Dang tai: Securemecca list"
		MPGETSSL ${u33} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		lognecho "> Dang tai: cryptomining and porn lists"
		MPGETSSL ${u34} | grep -o '^[^#]*' >> $hTam
		MPGETSSL ${u35} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		MPGETSSL ${u36} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		MPGETSSL ${u37} | grep -o '^[^#]*' >> $hTam
		lognecho "> Dang tai: Easylist & w3kbl lists"
		MPGETSSL ${u38} | grep -o '^[^#]*' >> $hTam
		MPGETSSL ${u39} | grep -o '^[^#]*' >> $hTam
		MPGETSSL ${u40} | grep -o '^[^#]*' >> $hTam
		MPGETSSL ${u41} | grep -o '^[^#]*' >> $hTam
		MPGETSSL ${u42} | grep -o '^[^#]*' >> $hTam
		MPGETSSL ${u43} | grep -o '^[^#]*' >> $hTam
		MPGETSSL ${u44} | grep -o '^[^#]*' >> $hTam
		MPGETSSL ${u45} | grep -o '^[^#]*' >> $hTam
		MPGETSSL ${u46} | grep -o '^[^#]*' >> $hTam
		MPGETSSL ${u47} | grep -o '^[^#]*' >> $hTam
		MPGETSSL ${u48} | grep -o '^[^#]*' >> $hTam
	fi
#___________________________________________________________________________________________________________________________________________________________________________________________________
	if [ $Level -ge 2 ]; then
		lognecho "# Unlocking Level=2 lists"
		lognecho "> Dang tai: even more StevenBlack lists"
		MPGETSSL ${u49} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		MPGETSSL ${u50} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		MPGETSSL ${u51} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		lognecho "> Dang tai: hosts-file EMD/FSA lists"
		MPGETSSL ${u52} | grep -o '^[^#]*' | grep -v "::" | awk '{print $2}' >> $hTam
		MPGETSSL ${u53} | grep -o '^[^#]*' | grep -v "::" | awk '{print $2}' >> $hTam
		lognecho "> Dang tai: hosts-file MMT/PHA lists"
		MPGETSSL ${u54} | grep -o '^[^#]*' | grep -v "::" | awk '{print $2}' >> $hTam
		MPGETSSL ${u55} | grep -o '^[^#]*' | grep -v "::" | awk '{print $2}' >> $hTam
		lognecho "> Dang tai: Cameleon list"
		MPGET ${u56} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		lognecho "> Dang tai: winhelp2002 list"
		MPGET ${u57} | grep -o '^[^#]*' | grep -v "::" | awk '{print $2}' >> $hTam
		lognecho "> Dang tai: someonewhocares list"
		MPGET ${u58} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		lognecho "> Dang tai: anudeepND lists"
		MPGETSSL ${u59} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		MPGETSSL ${u60} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		MPGETSSL ${u61} | grep -o '^[^#]*' >> $hTam
		lognecho "> Dang tai: CHEF-KOCH lists"
		MPGETSSL ${u62} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		MPGETSSL ${u63} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		MPGETSSL ${u64} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		MPGETSSL ${u65} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		MPGETSSL ${u66} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		lognecho "> Dang tai: joewein.de LLC list"
		MPGETSSL ${u67} | grep -o '^[^#]*' >> $hTam
		lognecho "> Dang tai: Windows telemetry lists"
		MPGETSSL ${u68} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		MPGETSSL ${u69} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		lognecho "> Dang tai: smart TV blocklists"
		MPGETSSL ${u70} | grep -o '^[^#]*' >> $hTam
		MPGETSSL ${u71} | grep -o '^[^#]*' >> $hTam
		lognecho "> Dang tai: a few more blocklists"
		MPGETSSL ${u72} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		MPGETSSL ${u73} | grep -o '^[^#]*' | grep -v "::" | awk '{print $2}' >> $hTam
		MPGETSSL ${u74} | grep -o '^[^#]*' >> $hTam
		MPGETSSL ${u75} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		MPGETSSL ${u76} | grep -o '^[^#]*' >> $hTam
		MPGETSSL ${u77} | grep -o '^[^#]*' >> $hTam
	fi
#___________________________________________________________________________________________________________________________________________________________________________________________________
	if [ $Level -ge 3 ]; then
		lognecho "# Unlocking Level=3 lists"
		lognecho "> Dang tai: hosts-file PSH/PUP/WRZ lists"
		MPGETSSL ${u78} | grep -o '^[^#]*' | grep -v "::" | awk '{print $2}' >> $hTam
		MPGETSSL ${u79} | grep -o '^[^#]*' | grep -v "::" | awk '{print $2}' >> $hTam
		lognecho "> Dang tai: Mahakala list"
		MPGETMHK ${u80} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		lognecho "> Dang tai: HostsFile.mine.nu list"
		MPGETSSL ${u81} | grep -o '^[^#]*' | awk '{print $2}' >> $hTam
		lognecho "> Dang tai: Kowabit list"
		MPGETSSL ${u82} | grep -o '^[^#]*' >> $hTam
		lognecho "> Dang tai: ADZHOSTS list"
		MPGETSSL ${u83} | grep -o '^[^#]*' | grep -v "::" | awk '{print $2}' >> $hTam
	fi
	...
	MPGETSSL ${HomePage}/denOn/denOn | grep -o '^[^#]*' > $denOn
	MPGETSSL ${HomePage}/trangOn/trangOn | grep -o '^[^#]*' > $trangOn
