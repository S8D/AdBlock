...
if [ -f $Lv0 ];then
	InRa "> Compacting Domains: [0]";
	cat $Lv0 | sed -r "${SedX}" | sed -r "${SedY}" > $tam;InRa ">> Compacted: $(Size "$tam")";cat $tam > $hTam;
	if [ -f "${TMuc}/Location" ];then
		cat $Lv0 > ${Dl}/Lv1.txt
	fi
	rm -f $Lv0;
...
if [ -f $Lv00 ];then
	InRa "> Compacting Domains: [0.5]";
	cat $Lv00 | tr '[:upper:]' '[:lower:]' | sed -r "${SedX}" | sed -r "${Sed00}" | sed -r "${Sed01}" | sed 's|\$.*$||' | grep -o '^[^|]*' > $tam;InRa ">> Compacted: $(Size "$tam")";cat $tam >> $hTam;
	if [ -f "${TMuc}/Location" ];then
...
if [ -f $Lv1 ];then
	InRa "> Compacting Domains: [1]";
	cat $Lv1 | sed -r "${SedX}" | sed -r "${Sed1}" | sed -r "${SedY}" > $tam;InRa ">> Compacted: $(Size "$tam")";cat $tam >> $hTam;
	if [ -f "${TMuc}/Location" ];then
		cat $Lv1 > ${Dl}/Lv1.txt
	fi
	rm -f $Lv1;
...
if [ -f $Lv2 ];then
	InRa "> Compacting Domains: [2]";
	cat $Lv2 | sed -r "${SedX}" | sed -r "${Sed2}" | sed -r "${SedY}" > $tam;InRa ">> Compacted: $(Size "$tam")";cat $tam >> $hTam;
	if [ -f "${TMuc}/Location" ];then
		cat $Lv2 > ${Dl}/Lv2.txt
	fi
	rm -f $Lv2;
...
if [ -f $Lv3 ];then
	InRa "> Compacting Domains: [3]";
	cat $Lv3 | sed -r "${SedX}" | sed -r "${Sed3}" | sed -r "${SedY}" > $tam;InRa ">> Compacted: $(Size "$tam")";cat $tam >> $hTam;
	if [ -f "${TMuc}/Location" ];then
		cat $Lv3 > ${Dl}/Lv3.txt
	fi
	rm -f $Lv3;
...
if [ -f $Lv4 ];then
	InRa "> Compacting Domains: [4]";
	cat $Lv4 | sed -r "${SedX}" | sed -r "${Sed4}" | sed -r "${SedY}" > $tam;InRa ">> Compacted: $(Size "$tam")";cat $tam >> $hTam;
	if [ -f "${TMuc}/Location" ];then
		cat $Lv4 > ${Dl}/Lv4.txt
	fi
	rm -f $Lv4;
