...
Sed4="s|^127\.0.*||; s|^0\.0\.0\.0||; s|^\s+||; s|^\-+||; s|^🔗||; s|^\_+||; s|:.*$||; s|\s+$||; s|^\.||; s|\.$||; s|\/.*$||; s|\.v$||; s|\.w$||; s|\.z$||; s|\+$||; s|\.tl2$|tl|; s|^w.*orn$||; s|\.co14$|.co|; s|\.world15$|.world|; s|com\.do.*ld$|com|; s/\.comm$/.com/; s|.*\-offer$||; s|.*ocom$||; s|.*ilien$||; s/\.com(1|a|en|br|ca|stu)[^.]*$/.com/"
SedY="s/.*[^a-z0-9]+$//; s|^[^.]+$||; /^$/d"
Dl="${TMuc}/dl";
