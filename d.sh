...
#grep -vE "(${SubDM})" | 
SedX="s|#.*$||; s|^0\.0\.0\.0||; s|^\s+||; s|^255.255.*||; "
Sed00="s|!.*$||; s/\|+//; s|\@.*$||; s|^127\.0.*||; s/^(\s+|\:\/+|\.|\*)+//; s|~.*$||; s|\^.*$||; s|\/.*$||; s|\}.*$||; s|\?.*$||; s|.*\.php$||; s|^\-||; s|.*\-tag.$||; s/\.$//; s|:.*$||; s|.*\.js$||; s|\&.*$||; s|.*\*.*||; s/\.com(stu|cate)[^.]*$/.com/; s|yandex$|yandex.com|; s|\.page\-w.*||"
Sed01="s/.*[^a-z0-9]+$//; s|.*\,.*||; s|\n.*\.$||; s/^[^.]+\.$//; s/\$.*//"
...
Sed2="s|:.*$||; s|\;.*$||; s|^127.0.0.1||; s|^\s+||; s|\s+$||; s|^0\.0\.0\.0||; s|^\s+||; s|^\_+||; s|\.k$||; s/\.comm$/.com/; s/\.com(1|a|en|stu|f4a|http|cate)[^.]*$/.com/; s|<.*$||; s|\.$||; s|^\_||; s|^\-||"
Sed3="s|^127.0.0.1||; s|^\s+||; s|\s+$||; s|^\-||; s|^\.||; s|:.*$||; s|\.$||; s|\?$||; s|^\_+||; s/\.com(en|stu|cate)[^.]*$/.com/; s/.*\.(1cn|4th)$//; s|^w.*orn$||; s|.*html$||"
Sed4="s|^127\.0.*||; s|^0\.0\.0\.0||; s|^\s+||; s|^\-+||; s|^🔗||; s|^\_+||; s|:.*$||; s|\s+$||; s|^\.||; s|\.$||; s|\/.*$||; s|\.v$||; s|\.w$||; s|\.z$||; s|\+$||; s|\.tl2$|tl|; s|^w.*orn$||; s|\.co14$|.co|; s|\.world15$|.world|; s|com\.do.*ld$|com|; s/\.comm$/.com/; s|.*\-offer$||; s|.*ocom$||; s|.*ilien$||; s/\.com(1|a|en|br|ca|stu)[^.]*$/.com/; s/.*[^a-z0-9]+$//"
SedY="s|^[^.]+$||; /^$/d"
Dl="${TMuc}/dl";
