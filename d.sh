...
SedX="s|#.*$||; s|<.*$||; s|\:.*$||; ";
SedY="s|^\.||; s|\.$||; s|^[^.]+$||; /^$/d";#s|^[[:space:]]*||g; 
Sed0="s|#.*$||; ";
# s|#.*$||; s|<.*$||; s|::||; s|0.0.0.0||; s|:.*$||; s|^127.0.0.1||; s|^255.255.*||; s|0.0.0.0||; s|\/$||; s|\.$||; s|^[[:space:]]*||g; s|\;|\n|; s|\s+$||; s|\^$||; s|^\-\-||; s|^\-||; s|^[^.]+$||; s|&.*$||; s|^🔗||; s|^\.||; s|^\_\_||g; s|^\_||g; s|\.$||; s|\?$||; s|\-$||; s|\+$||; s|\$.*$||; s|\.v$||; s|\.w$||; s|p.*mq$||; s|\.co.*rls$|.com|; /^$/d
#s|::||; s|^address=/||; s|0.0.0.0||; s|:.*$||; s|^127.0.0.1||; s|^255.255.*||; s|0.0.0.0||; s|\/$||; s|\.$||; s|^[[:space:]]*||g; s|\;|\n|; s|\s+$||; s|\^$||; s|^\-\-||; s|^\-||; s|^[^.]+$||; s|&.*$||; s|^🔗||; s|^\.||; s|^\_\_||g; s|^\_||g; s|\.$||; s|\?$||; s|\-$||; s|\+$||; s|\$.*$||; s|\.v$||; s|\.w$||; s|p.*mq$||; s|\.co.*rls$|.com|; /^$/d' | sed -r 's|\n^[^.]+$||; /^$/d' | awk '{if ($1 in a) next; a[$1]=$0; print}' > $tam

Sed00="s|^address=/||; ";
#s|!.*$||; s|#.*$||; s|:.*$||; s|\^.*$||; s|\@.*$||; s|\/.*$||; s|\?.*$||; s|.*\*.$||; s|~.*$||; s|0.0.0.0||; s|\,|\n|g; s|^127\.0\.0\.1.*$||; s/\|\|//; s|[[:blank:]]|\n|; s|^\.||; s|^[^.]+$||; s|>.*$||; s|\\.*$||; s|^\-||; s|\&.*$||; s|\*$||; s|^.*\.$||g; s|.*\*.*||; s|.*\.$||; s|\n.*\.$||' | sed -r 's|\.invalid$||; s|.*main$||; s|\.co.*orn$|.com|; s|^i.*nt$||; s|^s.*pt$||' | sed 's|\$.*$||' | grep -o '^[^|]*' | awk '{if ($1 in a) next; a[$1]=$0; print}' > $tam

Sed1="s|#.*$||; ";
#s|.*ating$||; s|.*kbang$||; s|.*ztop$||; s|.*kdns$||; s|.*main$||; s|\.v.*ost$|.vn|; s|\.co.*ost$|.com|; s|\.co.*ies$|.com|; s|\.co.*orn$|.com|' | sed -r '' | awk '{if ($1 in a) next; a[$1]=$0; print}' > $tam

Sed2="s|#.*$||; ";
#s|\.$||; s|\.k$||; s|\.co.*orn$|.com|; s|www$||' | sed -r 's|\n^[^.]+$||; /^$/d' | awk '{if ($1 in a) next; a[$1]=$0; print}' > $tam

Sed3="s|#.*$||; ";
#s|\.com.*orn$|.com|; s|^w.*orn$||; s|.*html$||' | sed -r 's|\n^[^.]+$||; /^$/d' | awk '{if ($1 in a) next; a[$1]=$0; print}' > $tam

Sed4="s|#.*$||; ";
#s|#.*$||; s|0.0.0.0||; s|0.0.0.0$||; s|.* br.*||; s|.* lo.*||; s|:.*$||; s|^[[:space:]]*||g; s|^\-\-||; s|^\-||; s|^\_\_||; s|^\_||; s|^🔗||; s|\/.*$||; s|\.com.*orn$|.com|; s|\.co.*rls$|.com|; s|^w.*orn$||; s|\.v$||; s|\.w$||; s|\.z$||; s|\+$||; s|\-$||; s|^[^.]+$||; /^$/d' | awk '{if ($1 in a) next; a[$1]=$0; print}' > $tam

#grep -vE "(${SubDM})" | 
#__________________________________________________________________________________________________
if [ -f $Lv0 ];then
	InRa "> Deleting Special character: [0]";
	cat $Lv0 | sed -r "${SedX}" | sed -r "${Sed0}" | sed -r "${SedY}" > $tam;Size $tam;cat $tam > $hTam;
	if [ -f "${TMuc}/Location" ];then
		cat $Lv1 > ${TMuc}/Lv1.txt
	fi
	rm -f $Lv0;
fi
if [ -f $Lv00 ];then
	InRa "> Deleting Special character: [0.5]";
	cat $Lv00 | tr '[:upper:]' '[:lower:]' | sed -r "${SedX}" | sed -r "${Sed00}" | sed -r "${SedY}" > $tam;Size $tam;cat $tam >> $hTam;
	if [ -f "${TMuc}/Location" ];then
		cat $Lv00 > ${TMuc}/Lv00.txt
	fi
	rm -f $Lv00;
fi
if [ -f $Lv1 ];then
	InRa "> Deleting Special character: [1]";
	cat $Lv1 | sed -r "${SedX}" | sed -r "${Sed1}" | sed -r "${SedY}" > $tam;Size $tam;cat $tam >> $hTam;
	if [ -f "${TMuc}/Location" ];then
		cat $Lv1 > ${TMuc}/Lv1.txt
	fi
	rm -f $Lv1;
...
if [ -f $Lv2 ];then
	InRa "> Deleting Special character: [2]";
	cat $Lv2 | sed -r "${SedX}" | sed -r "${Sed2}" | sed -r "${SedY}" > $tam;Size $tam;cat $tam >> $hTam;
	if [ -f "${TMuc}/Location" ];then
		cat $Lv2 > ${TMuc}/Lv2.txt
	fi
	rm -f $Lv2;
fi
if [ -f $Lv3 ];then
	InRa "> Deleting Special character: [3]";
	cat $Lv3 | sed -r "${SedX}" | sed -r "${Sed3}" | sed -r "${SedY}" > $tam;Size $tam;cat $tam >> $hTam;
	if [ -f "${TMuc}/Location" ];then
		cat $Lv3 > ${TMuc}/Lv3.txt
	fi
	rm -f $Lv3;
fi
if [ -f $Lv4 ];then
	InRa "> Deleting Special character: [4]";
	cat $Lv4 | sed -r "${SedX}" | sed -r "${Sed4}" | sed -r "${SedY}" > $tam;Size $tam;cat $tam >> $hTam;
	if [ -f "${TMuc}/Location" ];then
		cat $Lv4 > ${TMuc}/Lv4.txt
	fi
	rm -f $Lv4;
fi
