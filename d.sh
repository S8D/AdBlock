...
if [ -f $Lv3 ];then
	InRa "> Compacting [3]";
	cat $Lv3 | tr '[:upper:]' '[:lower:]' | sed -r "${SedX}" | sed -r "${Sed3}" | sed -r "${SedY}" > $tam;InRa ">> Compacted [3]: $(Size "$tam")";cat $tam >> $hTam;
	if [ -f "${TMuc}/Location" ];then
		cat $Lv3 > ${Dl}/Lv3.txt
	fi
	rm -f $Lv3;
...
if [ -f $Lv4 ];then
	InRa "> Compacting [4]";
	cat $Lv4 | tr '[:upper:]' '[:lower:]' | sed -r "${SedX}" | sed -r "${Sed4}" | sed -r "${SedY}" > $tam;InRa ">> Compacted [4]: $(Size "$tam")";cat $tam >> $hTam;
	if [ -f "${TMuc}/Location" ];then
		cat $Lv4 > ${Dl}/Lv4.txt
	fi
	rm -f $Lv4;
