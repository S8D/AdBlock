...
#grep -vE "(${SubDM})" | 
SedX="s|#.*$||; s|<.*$||; s|::||; s|^address=/||; s|0.0.0.0||; s|:.*$||; s|^127.0.0.1||; s|^255.255.*||; s|0.0.0.0||; s|\/$||; s|\.$||; s|^[[:space:]]*||g; s|\;|\n|; s|\s+$||; s|\^$||; s|^\-\-||; s|^\-||; s|^[^.]+$||; s|&.*$||; s|^🔗||; s|^\.||; s|^\_\_||g; s|^\_||g; s|\.$||; s|\?$||; s|\-$||; s|\+$||; s|\$.*$||; s|\.v$||; s|\.w$||; s|p.*mq$||; s|\.co.*rls$|.com|; s/\.com(1|a|m|au|br|en|f4a|http|categories)$/.com/; s|[[:space:]]$||; /^$/d";
...
if [ -f $Lv0 ];then
	InRa "> Compacting [0]";
	LC_ALL=C cat $Lv0 | sed -r "${SedX}" | sed -r "${SedY}" > $tam;InRa ">> Compacted [0]: $(Size "$tam")";cat $tam > $hTam;
	if [ -f "${TMuc}/Location" ];then
		cat $Lv0 > ${Dl}/Lv0.txt;cat $tam > ${Dl}/Lv0_.txt
	fi
	rm -f $Lv0;
fi
if [ -f $Lv00 ];then
	InRa "> Compacting [0.5]";
	LC_ALL=C cat $Lv00 | tr '[:upper:]' '[:lower:]' | sed -r "${SedX}" | sed -r "${Sed00}" | sed -r "${Sed01}" | sed 's|\$.*$||' | grep -o '^[^|]*' > $tam;InRa ">> Compacted [0.5]: $(Size "$tam")";cat $tam >> $hTam;
	if [ -f "${TMuc}/Location" ];then
		cat $Lv00 > ${Dl}/Lv00.txt;cat $tam > ${Dl}/Lv00_.txt
	fi
	rm -f $Lv00;
fi
if [ -f $Lv1 ];then
	InRa "> Compacting [1]";
	LC_ALL=C cat $Lv1 | sed -r "${SedX}" | sed -r "${Sed1}" | sed -r "${SedY}" > $tam;InRa ">> Compacted [1]: $(Size "$tam")";cat $tam >> $hTam;
	if [ -f "${TMuc}/Location" ];then
		cat $Lv1 > ${Dl}/Lv1.txt;cat $tam > ${Dl}/Lv1_.txt
	fi
	rm -f $Lv1;
fi
if [ -f $Lv2 ];then
	InRa "> Compacting [2]";
	LC_ALL=C cat $Lv2 | sed -r "${SedX}" | sed -r "${Sed2}" | sed -r "${SedY}" > $tam;InRa ">> Compacted [2]: $(Size "$tam")";cat $tam >> $hTam;
	if [ -f "${TMuc}/Location" ];then
		cat $Lv2 > ${Dl}/Lv2.txt;cat $tam > ${Dl}/Lv2_.txt
	fi
	rm -f $Lv2;
fi
if [ -f $Lv3 ];then
	InRa "> Compacting [3]";
	LC_ALL=C cat $Lv3 | tr '[:upper:]' '[:lower:]' | sed -r "${SedX}" | sed -r "${Sed3}" | sed -r "${SedY}" > $tam;InRa ">> Compacted [3]: $(Size "$tam")";cat $tam >> $hTam;
	if [ -f "${TMuc}/Location" ];then
		cat $Lv3 > ${Dl}/Lv3.txt;cat $tam > ${Dl}/Lv3_.txt
	fi
	rm -f $Lv3;
fi
if [ -f $Lv4 ];then
...
if [ ! -f $Lv00 ]||[ ! -f $Lv0 ]||[ ! -f $Lv1 ]||[ ! -f $Lv2 ]||[ ! -f $Lv3 ]||[ ! -f $Lv4 ];then
	...
	if [ -f "${TMuc}/Location" ];then
		cat $hChinh > ${Dl}/h.txt; cat $tam > ${Dl}/h_.txt; cat $dChinh > ${Dl}/d.txt
	fi
	rm -f $hChinh; rm -f $dChinh
fi
