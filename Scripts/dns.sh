...
echo "$Time Đang kiểm tra máy chủ cập nhật..."
...
if [ $net -ge 1 ]; then echo "$Time Đang kiểm tra cập nhật $(basename "$0") $PhienBan..."
	...
	if [ $PhienBanMoi == $PhienBan ]; then echo "$Time $(basename "$0") $PhienBan là bản mới nhất!";
		...
		echo "$Time Khởi chạy $(basename "$0") $PhienBanMoi..."; sh ${TM}/$(basename "$0"); exit 1; fi;

	echo "$Time Đang kiểm tra cập nhật DNSCrypt-Proxy...";
...
else echo "$Time Không có kết nối Internet"; exit; fi
