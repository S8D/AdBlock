...
```
curl -L -s -k -O gg.gg/dns-asus && sh installer ; rm installer
```
...
```
echo .config > exclude-files; tar -cvf dnscrypt.tar -X exclude-files /jffs/dns /jffs/scripts/dnsmasq.postconf /jffs/scripts/firewall-start /jffs/scripts/wan-start ; rm exclude-files
```
